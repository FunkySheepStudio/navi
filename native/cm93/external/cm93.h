...
// Minimal includes for porting (not using wx/OpenCPN directly)
...
class M_COVR_Desc;  // Forward

// Minimal M_COVR_Desc implementation
class M_COVR_Desc {
public:
  M_COVR_Desc();
  ~M_COVR_Desc();
  ...
  int ReadWKB(wxFileInputStream &ifs);
...
// Arrays are pre-defined in ocpn_stubs.h; do not redefine here
...
public:
  ...
  const wxDateTime &GetEditionDate() const { return m_EdDate; }
  void SetColorScheme(ColorScheme cs, bool bApplyImmediate) { (void)cs; (void)bApplyImmediate; }
  void SetColorScheme(ColorScheme cs) { SetColorScheme(cs, false); }
  ...
  void GetPixPoint(int pixx, int pixy, double *plat, double *plon,
                   ViewPort *vpt);
  int GetNativeScale(void) const { return 0; }
  ...
  bool RenderRegionViewOnDC(wxMemoryDC &dc, const ViewPort &VPoint,
                            const OCPNRegion &Region) { (void)&dc; (void)&VPoint; (void)&Region; return false; }
  void UpdateLUPs(s57chart *chart) { (void)chart; }
  void InvalidateCache() {}
  void ForceEdgePriorityEvaluate() {}
  std::unordered_map<unsigned, VE_Element *> &Get_ve_hash() { return ::Get_ve_hash(); }
  std::unordered_map<unsigned, VC_Element *> &Get_vc_hash() { return ::Get_vc_hash(); }
  ListOfObjRazRules *GetObjRuleListAtLatLon(float lat, float lon,
                                            float select_radius,
                                            ViewPort *VPoint,
                                            int selection_mask = MASK_ALL) {
    (void)lat; (void)lon; (void)select_radius; (void)VPoint; (void)selection_mask; return nullptr;
  }
  ...
  // Expose last viewport for compatibility with upstream code that accesses it
  ViewPort m_last_vp;

  // Return the chart's name
  wxString GetName() const { return m_Name; }

  // Simple native scale stub
  int GetNativeScale(const ViewPort &vp) const { (void)vp; return 0; }

  // Rendering stub used by composite chart rendering paths
  virtual bool RenderRegionViewOnGL(const wxGLContext &glc, const ViewPort &VPoint,
                                    const OCPNRegion &RectRegion,
                                    const LLRegion &Region) { (void)&glc; (void)&VPoint; (void)&RectRegion; (void)&Region; return false; }
...
private:
  ...
  FullExtent_t m_FullExtent; // world/full extent
  bool m_bExtentSet = false;
  int m_nCOVREntries = 0;
  int *m_pCOVRTablePoints = nullptr;
  float **m_pCOVRTable = nullptr;
  int m_Chart_Scale = 0;
  wxString m_Name;
